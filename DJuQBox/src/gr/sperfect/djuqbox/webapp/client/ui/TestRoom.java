...
public class TestRoom extends Composite implements HasText, PlayStateHandler {
		...
		final MessageArrivedEvent.Handler myh = new MessageArrivedEvent.Handler() {
			...
			public void onMessageArrived(MessageArrivedEvent e) {
				...
				LogMqtt("onMessageArrived: "
						+ e.getMessage().getDestinationName() + " | "
	...
	protected void handleMqttMessage(MqttMessage mqttMsg) {
		...
		if (mqttMsg.getDestinationName().startsWith(mqttRoomDest + "clients/")
				&& mqttMsg.getDestinationName().endsWith("/presense/")) {
	...
	public void setVideoFromSearch(String v) {
		...
		DJuQBox.API.getYoutubeMixForSong(v, new MethodCallback<YoutubePlayList>() {
			...
			@Override
			public void onSuccess(Method method, YoutubePlayList response) {
				// TODO Auto-generated method stub
				GWT.log("got mix");
				
				playList.loadPlayList(response);
			}
			
			@Override
			public void onFailure(Method method, Throwable exception) {
				// TODO Auto-generated method stub
				GWT.log("error loading mix" , exception);
			}
		});
